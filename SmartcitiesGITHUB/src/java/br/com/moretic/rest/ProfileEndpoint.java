...
public class ProfileEndpoint {
    ...
    /**
     * @TODO @return Response json array credicard user
     * @Todo conectar com a operadora de cartao para verificar se o cartao e
     * valido!!!
     */
    @GET
    @Path("/credicard/{number}/{month}/{year}/{postcode}/{name}/{surname}/{country}/{sec}/{idProfile}")
    @Produces("application/json")
    public Response paypal(@PathParam("number") String number,
            ...
            @PathParam("sec") String sec,
            @PathParam("idProfile") String idProfile,
            @Context HttpServletRequest req, @Context HttpServletResponse res) {

        Profile p = em.find(Profile.class, new Integer(idProfile));

        Query q = em.createQuery("SELECT c FROM Country c WHERE c.nmCountry like :pNameC");
        q.setParameter("pNameC", country);
        Country c = (Country) q.getSingleResult();

        q = em.createQuery("SELECT c FROM Credicard c WHERE c.credicardNumber = :pName", Credicard.class);
        q.setParameter("pName", number);

        Credicard card = null;
        try {
            card = (Credicard) q.getSingleResult();
        } catch (NoResultException e) {
            card = new Credicard();
        } finally {

            card.setCountry(c);
            card.setCredicardNumber(number);
            card.setMonth(month);
            card.setOwner(p);
            card.setYear(year);
            card.setIdProfile(p.getIdprofile());
            card.setIdCountry(c.getIdcountry());
            card.setP_surname(surname);
            card.setP_name(name);
            card.setPostingcode(postcode);
            card.setCredicardCode(sec);

            //Salva ou persiste
            em.merge(card);

            //recupera a lista de cards do cara
            q = em.createQuery("SELECT c FROM Credicard c WHERE c.idProfile = :pId");
            q.setParameter("pId", p.getIdprofile());
            List<Credicard> lCards = q.getResultList();
